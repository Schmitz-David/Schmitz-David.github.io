...
let boxWidth, boxHeight, b, padding, blinker, blinkerShown=false, fontSize, menuMaxHeight, smoothingFactor, listOfAirports, mustUpdateList=false, customFont;
...
function setup() {
  ...
  pixelDensity(1);
	boxWidth=min(windowWidth/5,80);
	boxHeight=min(windowHeight/5, 100);
	padding=20;
	b=[0,0,0];
  blinker=0;
  fontSize=boxHeight/4*3;
  menuMaxHeight=min(windowHeight/3*2,boxHeight*3);
  textSize(fontSize);
  textAlign(CENTER,CENTER);
  smoothingFactor=min(boxWidth,boxHeight)/5;
  
}

function draw() {
  if(blinker>0) blinker--;
  if(blinker==0) {blinker=50;
  blinkerShown=!blinkerShown;}
    
	background(255);
  for(let i=0;i<3;i++){
		noFill();
    rect((i+1)*padding+i*boxWidth,padding,boxWidth,boxHeight,smoothingFactor,smoothingFactor);
    fill(0);
		if(b[i]!=0) text(char(b[i]),(i+1)*padding+i*boxWidth,padding,boxWidth,boxHeight);
	}
  if(blinkerShown) drawCursor();
  
  if(getLastFilledIndex()>0) drawMenu();
}
function drawCursor(){
  let i=getLastFilledIndex();
  if(i==3) line(3*padding+2.9*boxWidth,padding+boxHeight*0.2,3*padding+2.9*boxWidth,padding+boxHeight*0.8);
  else line((i+1)*padding+(i+0.1)*boxWidth,padding+boxHeight*0.2,(i+1)*padding+(i+0.1)*boxWidth,padding+boxHeight*0.8);
}
...
function keyTyped(){
  let i=-1;
  if(keyCode==ENTER){
    if(listOfAirports.length==0) return;
    else{
      for(let i=0;i<3;i++) b[i]=listOfAirports[0][0].charCodeAt(i);
    }
    return;
  }
  if(keyCode>="a".charCodeAt(0)&&keyCode<="z".charCodeAt(0)){
    i=keyCode+"A".charCodeAt(0)-"a".charCodeAt(0);
  }
  if(keyCode>="A".charCodeAt(0)&&keyCode<="Z".charCodeAt(0)){
    i=keyCode;
  }
  if(i==-1) return;
  let j=getLastFilledIndex();
  if(j==3) j=2;
  b[j]=keyCode;
  mustUpdateList=true;
}
...
function updateListOfAirports(){
  listOfAirports=[];
  let currentString="",index=0;
  while(index<3&&b[index]!=0){
    currentString+=char(b[index]);
    index++;
  }
 
  for(const a of airports){
    if(a[0].substring(0,index)==currentString){
      listOfAirports.push(a);
    }
  }
}
function getTextWidth(string, fontsz){
  return string.length;
}
function drawMenu(){
  if(mustUpdateList){
    updateListOfAirports();
    mustUpdateList=false;
  }
  if(listOfAirports.length==0){
    noFill();
    rect(padding,2*padding+boxHeight,2*padding+3*boxWidth,boxWidth,smoothingFactor,smoothingFactor);
    textSize(fontSize/3);
    fill(255,0,0);
    text("No airport found",padding,2*padding+boxHeight,2*padding+3*boxWidth,boxWidth);
    fill(0);
    textSize(fontSize);
    return;
  }
noFill();
let actualHeight=min(menuMaxHeight,boxHeight/3*listOfAirports.length), actualWidth=2*padding+3*boxWidth;
  rect(padding,2*padding+boxHeight, actualWidth,actualHeight,smoothingFactor,smoothingFactor);
  let usedHeight=0, airportString;
  textSize(fontSize/3);
  for(const airport of listOfAirports){
    
    usedHeight+=boxHeight/3;
    if(usedHeight>actualHeight) break;
    fill(0);
    airportString=airport[0]+": "+airport[1];
    if(getTextWidth(airportString,fontSize/3)>20){
       airportString=airportString.substring(0,18)+"...";
    }
    text(airportString,padding,2*padding+boxHeight+usedHeight-boxHeight/3,
        2*padding+3*boxWidth,boxHeight/3);
  }
  textSize(fontSize);
}
